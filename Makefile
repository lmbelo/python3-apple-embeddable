...
PYTHON_INSTALL-$(target)=$(PROJECT_DIR)/install/$(os)/macosx/python-$(PYTHON_VERSION)
PYTHON_FRAMEWORK-$(target)=$$(PYTHON_INSTALL-$(target))/Python.framework
PYTHON_LIB-$(target)=$$(PYTHON_FRAMEWORK-$(target))/Python
PYTHON_INSTALL_UNIVERSAL=$(PROJECT_DIR)/install/macOS/macosx/python-$(PYTHON_VERSION)/Python.framework/Versions/$(PYTHON_VER)
endif
...
dist/Python-$(PYTHON_VER)-macOS-support.$(BUILD_NUMBER).tar.gz: \
	...
	rm -R $$(PYTHON_INSTALL_UNIVERSAL)/Resources
	rm $$(PYTHON_INSTALL_UNIVERSAL)/Python

	cd $$(PYTHON_INSTALL_UNIVERSAL) && zip -r $(PROJECT_DIR)/dist/python3-macos-$(PYTHON_VERSION)-universal2.zip *
else
